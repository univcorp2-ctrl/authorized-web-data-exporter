...
`web-export export` はスクレイピング後に `outputs/investment_analysis.xlsx` を自動生成します。既存の `outputs/records.jsonl` から再分析する場合は次を実行します。
